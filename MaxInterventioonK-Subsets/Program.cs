...
namespace MaxInterventionK_Subsets
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine(
                    "Usage: MaxInterventioonK-Subsets.exe <folder-path>");
            ...
            {
                Console.WriteLine(
                    "The specified folder does not exist: " + folderPath);

                return;
            }

            string[] instanceFiles = Directory.GetFiles(
                folderPath,
                "*.txt",
            ...
            {
                Console.WriteLine(
                    "The folder does not contain any .txt files.");
        ...
        private static void ExecuteInstance(string filePath)
        {
            ...
            try
            {
                ...
                Queue<IAlgorithm> algorithms =
                    new Queue<IAlgorithm>();
                ...
                while (algorithms.Count > 0)
                {
                    ...
                    string result =
                        algorithm.PrintSummarySolution();
        ...
        private static void SaveResults(
            ...
            Queue<string> dataCollected)
        {
            string instanceFolder =
                Path.GetDirectoryName(instanceFilePath);

            string resultsFolder =
                Path.Combine(instanceFolder, "results");

            Directory.CreateDirectory(resultsFolder);

            string resultFileName =
                Path.GetFileNameWithoutExtension(instanceFilePath)
                + "_results.txt";

            string resultFilePath =
                Path.Combine(resultsFolder, resultFileName);

            string resultText =
                string.Concat(dataCollected);
